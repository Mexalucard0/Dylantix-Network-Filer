...
RegisterServerEvent("vrp_inventory:openGui")
AddEventHandler(
    ...
    function()
        ...
        if data and data.inventory then
            ...
            TriggerClientEvent("vrp_inventory:updateInventory", source, inventory, weight, maxWeight)
        end
    end
)

RegisterServerEvent("vrp_inventory:useItem")
AddEventHandler(
    "vrp_inventory:useItem",
    function(args)
        local data = args[1]
        local user_id = vRP.getUserId({source})
        local player = vRP.getUserSource({user_id})
...
RegisterServerEvent("vrp_inventory:dropItem")
AddEventHandler(
    "vrp_inventory:dropItem",
    function(data)
        local user_id = vRP.getUserId({source})
        local player = vRP.getUserSource({user_id})
        local amount = parseInt(data.amount)
        if vRP.tryGetInventoryItem({user_id, data.idname, amount, false}) then
            TriggerClientEvent("vrp_inventory:closeGui", player)
    ...
    if type == "drink" or type == "food" or type == "heal" or type == "weapon" or type == "ammo" then
        if type == "drink" then
            if vRP.tryGetInventoryItem({user_id, idname, tonumber(amount), false}) then
                TriggerClientEvent("vrp_inventory:objectForAnimation", player, "prop_ld_flow_bottle")
                play_drink(player)
        ...
        if type == "food" then
            if vRP.tryGetInventoryItem({user_id, idname, tonumber(amount), false}) then
                TriggerClientEvent("vrp_inventory:objectForAnimation", player, "prop_cs_burger_01")
                play_eat(player)
                for i = 1, tonumber(amount) do
...
RegisterServerEvent("vrp_inventory:giveItem")
AddEventHandler(
    "vrp_inventory:giveItem",
    function(data)
        local user_id = vRP.getUserId({source})
        local player = vRP.getUserSource({user_id})
        if user_id ~= nil then
