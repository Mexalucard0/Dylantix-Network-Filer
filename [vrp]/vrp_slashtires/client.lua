Citizen.CreateThread(function()
	while true do
		local allowedWeapons = {"WEAPON_KNIFE"}
			...
			if CanUseWeapon(allowedWeapons) then
				...
				if closestTire ~= nil then
					...
					if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
						Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, tostring("[~g~E~w~] Slash dæk"))
...
AddEventHandler("SlashTires:SlashClientTire", function(tireIndex)
	TriggerEvent("chatMessage", "^1En spiller forsøger at skære dit dæk")
	local player = PlayerId()
	local plyPed = GetPlayerPed(player)
