          ...
          if not banned then
            ...
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then -- not present on the server, init
                  ...
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(user_id, function(last_login)
                      ...
                      local dmessage = "**ID:".. tostring(user_id).. " connected!**"
                      PerformHttpRequest('https://discordapp.com/api/webhooks/474022844402106369/_jYTe3_UATILWFvp14SGRLJwDptMN-xMKr9KGFlODbf2gdw9afyx5VqkfdT7vGBp9hWJ', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
                  ...
                  local dname = "[ID - ".. tostring(user_id).. "] SP-Logging"
                  local dmessage = "*ID:".. tostring(user_id).. " rejoined!*"
                  PerformHttpRequest('https://discordapp.com/api/webhooks/474022844402106369/_jYTe3_UATILWFvp14SGRLJwDptMN-xMKr9KGFlODbf2gdw9afyx5VqkfdT7vGBp9hWJ', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
                ...
                deferrals.done("Ansøg om whitelist, discord: https://discord.gg/k6HYNBa (user_id = "..user_id..").")
              end
            end)
          else
...
AddEventHandler("playerDropped",function(reason)
  ...
  if user_id ~= nil then
    ...
    local dname = "[ID - ".. tostring(user_id).. "] SP-Logging"
    local dmessage = "ID:".. tostring(user_id).. " disconnected!"
    PerformHttpRequest('https://discordapp.com/api/webhooks/474022844402106369/_jYTe3_UATILWFvp14SGRLJwDptMN-xMKr9KGFlODbf2gdw9afyx5VqkfdT7vGBp9hWJ', function(err, text, headers) end, 'POST', json.encode({username = dname, content = dmessage}), { ['Content-Type'] = 'application/json' })
