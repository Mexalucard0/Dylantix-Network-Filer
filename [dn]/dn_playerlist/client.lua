vRPsb = {}
Tunnel.bindInterface("dn_playerlist1",vRPsb)
users = {}
isEmergency = false
playerBlips = {}

RegisterNetEvent("dn:playerlist")
AddEventHandler("dn:playerlist", function(user_id,data)
  ...
  while true do
    ...
    if IsControlPressed(0, 27)--[[ INPUT_PHONE ]] then
      ...
      elseif v.job == "Elev" then
        ...
        lawyer = lawyer+1
      elseif v.job == "Advokat Chef" then
				lawyer = lawyer+1
        ...
        mechanic = mechanic+1
      elseif v.job == "Mekaniker Chef" then
        mechanic = mechanic+1
		...
		table.insert(players,'<tr class="header"><td colspan="5"><table width="100%"><tr><td class="job">🚑 '..ems..'</td><td class="job">👮 '..police..'</td><td class="job">💼 '..lawyer..'</td><td class="job">🧠 '..psycho..'</td><td class="job">🔧 '..mechanic..'</td><td class="job">🚖 '..taxi..'</td><td class="job">✅ '..playersOn..'/32</td><tr></table></td></tr>')
        ...
        elseif v.job == "Mekaniker Chef" then jobicon = "🔧"
        elseif v.job == "Advokat" then jobicon = "💼"
        elseif v.job == "Advokat Chef" then jobicon = "💼"
      ...
      if v.source ~= nil and NetworkIsPlayerActive(GetPlayerFromServerId(v.source)) then
        ...
        if distance < 15 and GetPlayerPed(-1) ~= GetPlayerPed(GetPlayerFromServerId(v.source)) and IsEntityVisible(GetPlayerPed(GetPlayerFromServerId(v.source))) then
          ...
          if NetworkIsPlayerTalking(GetPlayerFromServerId(v.source)) then
            --DrawMarker(2,x2,y2,z2+1.0,0,0,0,0,0,GetEntityHeading(GetPlayerPed(GetPlayerFromServerId(v.source))),0.25,-0.1,-0.25,150,0,0,100,0,0,2,0,0,0,90)
          else
          --  DrawMarker(2,x2,y2,z2+1.0,0,0,0,0,0,GetEntityHeading(GetPlayerPed(GetPlayerFromServerId(v.source))),0.25,-0.1,-0.25,0,150,0,100,0,0,2,0,0,0,90)
          end 
        end
