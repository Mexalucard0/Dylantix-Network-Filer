	...
	if car then
      ...
      if (AntiCheat == true)then
		if isCarBlacklisted(carModel) then
			_DeleteEntity(car)
			TriggerServerEvent("dn_ch:Cars", blacklistedCar )
        end
      end
	end
end
	...
	while true do
		...
		if IsPedJumping(PlayerPedId()) then
			...
			if jumplength > 250 then
				TriggerServerEvent("dn_ch:Jump", jumplength )
			end
        end
    end
	end
...
RegisterNetEvent("dn_ch:Toggle")
AddEventHandler("dn_ch:Toggle", function()
...
Citizen.CreateThread(function()
	while true do
		Wait(600000)
        TriggerEvent("chatMessage", "^3[CHEATER]", {255, 255, 0}, "^1 SERVEREN HAR ^3FUNDET EN CHEATER!")
	end
end)
